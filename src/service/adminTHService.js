...
export const createNewRoomTHService = async (req, res) => {
    ...
    await pool.execute(
        'insert into phongthuchanh(roleid,tenphong, maphong, mota, chuyenkhoa, soluongsinhvien) values (?, ?, ?, ?, ?, ?)',
...
export const postEditRoomService = async (req, res) => {
    ...
    await pool.execute(
        'update phongthuchanh set roleId = ?,tenphong=?, maphong=? , mota = ?, chuyenkhoa= ?, soluongsinhvien=? where id = ?',
